        ...
        <div className={cn('block', 'md:hidden', 'min-h-screen', 'bg-gradient-to-br', 'from-blue-600', 'via-blue-600', 'to-blue-700', 'relative', 'overflow-hidden')}>
            ...
            <div className={cn('relative', 'z-10', 'px-4', 'py-6', 'safe-area-inset')}>
                ...
                >
                    ...
                    <motion.h1
                        key={`title-${serviceType}`}
                        initial={{ opacity: 0, y: -10 }}
                        animate={{ opacity: 1, y: 0 }}
                        transition={{ duration: 0.3 }}
                        className={cn('text-3xl', 'font-extrabold', 'text-white', 'mb-2', 'drop-shadow-md')}
                    >
                        {serviceType === "helicopter" ? "Book Your Helicopter" :
                            serviceType === "hotel" ? "Book Your Hotel" :
                                serviceType === "package" ? "Holiday Packages" :
                                    serviceType === "schedule" ? "Weekly Schedule" :
                                        "Book Your Flight"}
                    </motion.h1>
                    ...
                    >
                        {serviceType === "helicopter" ? "Experience luxury helicopter travel" :
                            serviceType === "hotel" ? "Find perfect stays at best prices" :
                                serviceType === "package" ? "Unforgettable journeys await you" :
                                    serviceType === "schedule" ? "Plan your week in advance" :
                ...
                >
                    <div className="relative group">
                        <div className="absolute inset-y-0 left-0 pl-5 flex items-center pointer-events-none">
                            <FaSuitcase className="h-5 w-5 text-gray-400 group-focus-within:text-blue-500 transition-colors" />
                        </div>
                        <input
                            type="text"
                            className="block w-full pl-14 pr-5 py-4 bg-white/95 backdrop-blur-md border-0 rounded-2xl text-gray-900 placeholder-gray-500 shadow-xl focus:ring-2 focus:ring-blue-500 transition-all text-lg font-medium"
                ...
                >
                    <Card className={cn('bg-white', 'backdrop-blur-xl', 'shadow-2xl', 'rounded-3xl', 'border', 'border-white/30', 'overflow-visible')}>
                        <CardContent className="p-1">
                            {/* Service Type Toggle */}
                            <div className={cn('flex', 'flex-wrap', 'gap-1.5', 'mb-5', 'p-1', 'bg-gray-100', 'rounded-2xl', 'justify-center')}>
                                {[
                                    { id: "flight", label: "Flight", icon: FaPlane, color: "text-blue-600" },
                                    { id: "helicopter", label: "Heli", icon: FaHelicopter, color: "text-red-600" },
                                    { id: "hotel", label: "Hotel", icon: FaHome, color: "text-purple-600" },
                                    { id: "package", label: "Package", icon: FaSuitcase, color: "text-emerald-600" },
                                    { id: "schedule", label: "Schedule", icon: FaClock, color: "text-orange-600" }
                                ].map((tab) => (
                                    <button
                                        key={tab.id}
                                        onClick={() => {
                                            setServiceType(tab.id);
                                            if (tab.id === "flight" || tab.id === "helicopter") {
                                                setDeparture("");
                                                setArrival("");
                                            }
                                        }}
                                        className={`flex-1 min-w-[55px] py-2.5 px-1.5 rounded-xl font-bold text-[10px] transition-all duration-300 flex flex-col items-center justify-center gap-1 ${serviceType === tab.id
                            ...
                            <div className="space-y-5">
                                ...
                                {serviceType === "hotel" && <MobileHotelBooking />}
                                {serviceType === "package" && (
                                ...
                                {serviceType === "schedule" && <ComingSoonService serviceName="Weekly Schedule" />}
                                {(serviceType === "flight" || serviceType === "helicopter") && (
                                    <>
                                        {/* From/To Section */}
