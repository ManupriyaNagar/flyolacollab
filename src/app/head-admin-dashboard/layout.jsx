...
export default function HeadAdminDashboardLayout({ children }) {
  ...
  useEffect(() => {
    if (!authState.isLoading && (!authState.isLoggedIn || authState.userRole !== '4')) {
      router.push('/sign-in');
    }
  }, [authState, router]);
  ...
  const isActive = (href) => {
    ...
    if (normalizedHref === "/head-admin-dashboard") {
  ...
  if (authState.isLoading) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-100">
        <div className="flex">
          <div className="w-80 bg-gradient-to-br from-slate-900 via-blue-900 to-indigo-900 min-h-screen p-6">
  ...
  if (!authState.isLoggedIn || authState.userRole !== '4') {
    return null;
  }
        ...
        >
          ...
          <div className="p-6 border-b border-slate-700/50">
            ...
            <div className="flex items-center gap-3 p-3 bg-slate-800/50 rounded-xl">
              ...
              <div className="flex-1">
                <p className="text-sm font-medium text-white">Head Administrator</p>
                <p className="text-xs text-slate-400">Executive Level</p>
              </div>
            </div>
          </div>
          ...
          <nav className="flex-1 p-4 space-y-2 overflow-y-auto">
            ...
            <div className="mb-6">
              ...
              <Link
                href="/head-admin-dashboard"
                className={`flex items-center gap-3 py-3 px-4 rounded-xl transition-all duration-200 group ${
                  isActive("/head-admin-dashboard") 
                    ? "bg-gradient-to-r from-blue-600 to-indigo-600 text-white shadow-lg" 
                    : "hover:bg-slate-800/50 text-slate-300 hover:text-white"
                }`}
              >
                <FaHome className={`text-lg ${isActive("/head-admin-dashboard") ? "text-white" : "text-blue-400"}`} />
            ...
            <div className="mb-6">
              ...
              <div className="space-y-1">
                <Link
                  href="/head-admin-dashboard/user-management"
                  className={`flex items-center gap-3 py-3 px-4 rounded-xl transition-all duration-200 group ${
                    isActive("/head-admin-dashboard/user-management") 
                      ? "bg-gradient-to-r from-blue-600 to-indigo-600 text-white shadow-lg" 
                      : "hover:bg-slate-800/50 text-slate-300 hover:text-white"
                  }`}
                >
                  <FaUsers className={`text-lg ${isActive("/head-admin-dashboard/user-management") ? "text-white" : "text-emerald-400"}`} />
                ...
                <Link
                  href="/head-admin-dashboard/system-reports"
                  className={`flex items-center gap-3 py-3 px-4 rounded-xl transition-all duration-200 group ${
                    isActive("/head-admin-dashboard/system-reports") 
                      ? "bg-gradient-to-r from-blue-600 to-indigo-600 text-white shadow-lg" 
                      : "hover:bg-slate-800/50 text-slate-300 hover:text-white"
                  }`}
                >
                  <FaFileAlt className={`text-lg ${isActive("/head-admin-dashboard/system-reports") ? "text-white" : "text-purple-400"}`} />
            ...
            <div className="flex items-center justify-between px-6 py-4">
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="flex items-center gap-2 px-3 py-2 bg-gradient-to-r from-blue-500 to-indigo-500 rounded-xl text-white">
                  <User size={18} />
                  <span className="text-sm font-medium hidden sm:block">Head Admin</span>
                </div>
              </div>
            </div>
          </header>
