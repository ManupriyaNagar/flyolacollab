...
export default function DirectorAdminDashboardLayout({ children }) {
  ...
  useEffect(() => {
    if (!authState.isLoading && (!authState.isLoggedIn || authState.userRole !== '6')) {
      router.push('/sign-in');
    }
  }, [authState, router]);
  ...
  const isActive = (href) => {
    ...
    if (normalizedHref === "/director-admin-dashboard") {
  ...
  if (authState.isLoading) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-100">
        <div className="flex">
          <div className="w-80 bg-gradient-to-br from-blue-900 via-indigo-900 to-purple-900 min-h-screen p-6">
  ...
  if (!authState.isLoggedIn || authState.userRole !== '6') {
    return null;
  }
    ...
    <RouteGuard>
      <div className="flex min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-100">
        ...
        <aside
          ...
          } md:translate-x-0 bg-gradient-to-br from-blue-900 via-indigo-900 to-purple-900 text-white shadow-2xl border-r border-slate-700/50`}
        >
          {/* Header */}
          <div className="p-6 border-b border-slate-700/50">
            ...
            <div className="flex items-center gap-3 p-3 bg-slate-800/50 rounded-xl">
              ...
              <div className="flex-1">
                ...
                <p className="text-xs text-slate-400">Operations Level</p>
              </div>
            </div>
          </div>
          ...
          <nav className="flex-1 p-4 space-y-2 overflow-y-auto">
            ...
            <div className="mb-6">
              ...
              <Link
                href="/director-admin-dashboard"
                className={`flex items-center gap-3 py-3 px-4 rounded-xl transition-all duration-200 group ${
                  isActive("/director-admin-dashboard") 
                    ? "bg-gradient-to-r from-blue-600 to-purple-600 text-white shadow-lg" 
                    : "hover:bg-slate-800/50 text-slate-300 hover:text-white"
                }`}
              >
                <FaHome className={`text-lg ${isActive("/director-admin-dashboard") ? "text-white" : "text-blue-400"}`} />
            ...
            <div className="mb-6">
              ...
              <div className="space-y-1">
                <Link
                  href="/director-admin-dashboard/flight-operations"
                  className={`flex items-center gap-3 py-3 px-4 rounded-xl transition-all duration-200 group ${
                    isActive("/director-admin-dashboard/flight-operations") 
                      ? "bg-gradient-to-r from-blue-600 to-purple-600 text-white shadow-lg" 
                      : "hover:bg-slate-800/50 text-slate-300 hover:text-white"
                  }`}
                >
                  <FaPlane className={`text-lg ${isActive("/director-admin-dashboard/flight-operations") ? "text-white" : "text-emerald-400"}`} />
                ...
                <Link
                  href="/director-admin-dashboard/performance-analytics"
                  className={`flex items-center gap-3 py-3 px-4 rounded-xl transition-all duration-200 group ${
                    isActive("/director-admin-dashboard/performance-analytics") 
                      ? "bg-gradient-to-r from-blue-600 to-purple-600 text-white shadow-lg" 
                      : "hover:bg-slate-800/50 text-slate-300 hover:text-white"
                  }`}
                >
                  <FaChartLine className={`text-lg ${isActive("/director-admin-dashboard/performance-analytics") ? "text-white" : "text-yellow-400"}`} />
            ...
            <div className="flex items-center justify-between px-6 py-4">
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="flex items-center gap-2 px-3 py-2 bg-gradient-to-r from-blue-500 to-purple-500 rounded-xl text-white">
                  <User size={18} />
                  <span className="text-sm font-medium hidden sm:block">Director</span>
                </div>
              </div>
            </div>
          </header>
