...
export default function ChairmanAdminDashboardLayout({ children }) {
  ...
  useEffect(() => {
    if (!authState.isLoading && (!authState.isLoggedIn || authState.userRole !== '5')) {
      router.push('/sign-in');
    }
  }, [authState, router]);
  ...
  const isActive = (href) => {
    ...
    if (normalizedHref === "/chairman-admin-dashboard") {
  ...
  if (authState.isLoading) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-100">
        <div className="flex">
          <div className="w-80 bg-gradient-to-br from-indigo-900 via-purple-900 to-blue-900 min-h-screen p-6">
  ...
  if (!authState.isLoggedIn || authState.userRole !== '5') {
    return null;
  }
        ...
        >
          ...
          <div className="p-6 border-b border-slate-700/50">
            ...
            <div className="flex items-center gap-3 p-3 bg-slate-800/50 rounded-xl">
              ...
              <div className="flex-1">
                <p className="text-sm font-medium text-white">Chairman</p>
                <p className="text-xs text-slate-400">Board Level</p>
              </div>
            </div>
          </div>
          ...
          <nav className="flex-1 p-4 space-y-2 overflow-y-auto">
            ...
            <div className="mb-6">
              ...
              <Link
                href="/chairman-admin-dashboard"
                className={`flex items-center gap-3 py-3 px-4 rounded-xl transition-all duration-200 group ${
                  isActive("/chairman-admin-dashboard") 
                    ? "bg-gradient-to-r from-purple-600 to-indigo-600 text-white shadow-lg" 
                    : "hover:bg-slate-800/50 text-slate-300 hover:text-white"
                }`}
              >
                <FaHome className={`text-lg ${isActive("/chairman-admin-dashboard") ? "text-white" : "text-purple-400"}`} />
            ...
            <div className="mb-6">
              ...
              <div className="space-y-1">
                <Link
                  href="/chairman-admin-dashboard/strategic-reports"
                  className={`flex items-center gap-3 py-3 px-4 rounded-xl transition-all duration-200 group ${
                    isActive("/chairman-admin-dashboard/strategic-reports") 
                      ? "bg-gradient-to-r from-purple-600 to-indigo-600 text-white shadow-lg" 
                      : "hover:bg-slate-800/50 text-slate-300 hover:text-white"
                  }`}
                >
                  <FaChartBar className={`text-lg ${isActive("/chairman-admin-dashboard/strategic-reports") ? "text-white" : "text-emerald-400"}`} />
                  <span className="font-medium">Strategic Reports</span>
                  {isActive("/chairman-admin-dashboard/strategic-reports") && (
                    <div className="ml-auto w-2 h-2 bg-white rounded-full"></div>
                  )}
                </Link>
                ...
                <Link
                  href="/chairman-admin-dashboard/policy-management"
                  className={`flex items-center gap-3 py-3 px-4 rounded-xl transition-all duration-200 group ${
                    isActive("/chairman-admin-dashboard/policy-management") 
                      ? "bg-gradient-to-r from-purple-600 to-indigo-600 text-white shadow-lg" 
                      : "hover:bg-slate-800/50 text-slate-300 hover:text-white"
                  }`}
                >
                  <FaFileContract className={`text-lg ${isActive("/chairman-admin-dashboard/policy-management") ? "text-white" : "text-yellow-400"}`} />
          ...
          <header className="sticky top-0 z-20 bg-white/80 backdrop-blur-lg border-b border-slate-200/50 shadow-sm">
            <div className="flex items-center justify-between px-6 py-4">
              ...
              <div className="flex items-center gap-3">
                ...
                {/* Profile */}
                <div className="flex items-center gap-2 px-3 py-2 bg-gradient-to-r from-purple-500 to-indigo-500 rounded-xl text-white">
                  <User size={18} />
                  <span className="text-sm font-medium hidden sm:block">Chairman</span>
                </div>
              </div>
            </div>
          </header>
