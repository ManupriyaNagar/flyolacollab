...
const AddHotel = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const [hotelsRes, citiesRes, mealPlansRes] = await Promise.all([
  ...
  const handleSubmitHotel = async (e) => {
    ...
    try {
      const response = await API.hotels.createHotel(formData);
        ...
        <div className={cn('max-w-7xl', 'mx-auto', 'px-4', 'sm:px-6', 'lg:px-8', 'py-6')}>
          <div className={cn('flex', 'items-center', 'gap-4')}>
            ...
            <div>
              ...
              <p className={cn('text-gray-600', 'mt-1')}>Create a new hotel property with all details</p>
            </div>
          </div>
        </div>
      </div>
        ...
        <form onSubmit={handleSubmitHotel} className={cn('space-y-8')}>
          ...
          <div className={cn('flex', 'justify-end', 'gap-4', 'pt-8', 'border-t-2', 'border-gray-200', 'bg-white', 'rounded-xl', 'p-8', 'shadow-lg')}>
            ...
            <button
              type="submit"
              disabled={submitting}
              className={cn('px-10', 'py-3', 'bg-gradient-to-r', 'from-blue-600', 'to-indigo-600', 'text-white', 'font-bold', 'rounded-xl', 'hover:from-blue-700', 'hover:to-indigo-700', 'transition-all', 'duration-200', 'disabled:opacity-50', 'disabled:cursor-not-allowed', 'flex', 'items-center', 'gap-3', 'shadow-lg', 'hover:shadow-xl', 'transform', 'hover:scale-105')}
            >
              {submitting ? (
                <>
