...
export default function HelicopterManagementPage() {
  ...
  const fetchHelicopters = async () => {
    setLoading(true);
    try {
      // Fetch helicopters and airports (with helipad data)
      const [helicoptersRes, airportsRes] = await Promise.all([
  ...
  const filteredHelicopters = useMemo(() => {
    ...
    return filtered;
  }, [helicopters, searchTerm, sortConfig]);
  ...
  const getSortIcon = (columnKey) => {
    ...
    return sortConfig.direction === 'asc' ? 
      <ArrowsUpDownIcon className="w-4 h-4 text-red-500 rotate-180" /> :
      <ArrowsUpDownIcon className="w-4 h-4 text-red-500" />;
  };
        ...
        <div>
          <h1 className="text-3xl font-bold text-slate-800 flex items-center gap-3">
            <div className="p-2 bg-gradient-to-r from-red-500 to-pink-500 rounded-xl">
              <CogIcon className="w-8 h-8 text-white rotate-45" />
            </div>
            Helicopter Management
          </h1>
          <p className="text-slate-600 mt-2">Manage your helicopter fleet and specifications</p>
        </div>
        
        <button
          onClick={() => setShowModal(true)}
          className="flex items-center gap-2 px-6 py-3 bg-gradient-to-r from-red-500 to-pink-500 text-white rounded-xl hover:from-red-600 hover:to-pink-600 transition-all duration-200 shadow-lg font-medium"
        >
        ...
        <div className="flex flex-col md:flex-row gap-4">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-3 border border-slate-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-transparent transition-all"
      ...
      <div className="bg-white rounded-2xl shadow-lg border border-slate-200 overflow-hidden">
        <div className="bg-gradient-to-r from-red-50 to-pink-50 px-6 py-4 border-b border-slate-200">
          <h3 className="text-xl font-semibold text-slate-800 flex items-center gap-2">
            <CogIcon className="w-6 h-6 text-red-600 rotate-45" />
            ...
            <tbody className="divide-y divide-slate-100 bg-white">
              {loading ? (
                ...
                <tr>
                  <td colSpan={6} className="px-6 py-12 text-center">
                    <div className="flex flex-col items-center gap-3">
                      <CogIcon className="w-12 h-12 text-slate-300 rotate-45" />
                      <div>
                        <p className="text-slate-500 font-medium">No helicopters found</p>
                ...
                filteredHelicopters.map((helicopter) => (
                  <tr key={helicopter.id} className="hover:bg-slate-50/50 transition-colors">
                    <td className="px-4 py-3 whitespace-nowrap">
                      <div className="font-semibold text-slate-900 text-sm">{helicopter.helicopter_number}</div>
                    </td>
                    <td className="px-4 py-3 whitespace-nowrap">
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-1.5 text-xs min-w-[250px]">
                        ...
                        <span className="text-slate-700 truncate" title={getHelipadName(helicopter.start_helipad_id) || 'Not set'}>
                          {getHelipadName(helicopter.start_helipad_id) || 'Not set'}
                        </span>
                        <span className="text-slate-400 mx-1">→</span>
                        <MapPinIcon className="w-3 h-3 text-red-500 flex-shrink-0" />
                        <span className="text-slate-700 truncate" title={getHelipadName(helicopter.end_helipad_id) || 'Not set'}>
              ...
              <Dialog.Panel className="w-full max-w-4xl bg-white rounded-2xl shadow-2xl max-h-[90vh] overflow-y-auto">
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-6">
                  ...
                  <div className="flex justify-end gap-3 pt-4">
                    ...
                    <button
                      type="submit"
                      className="px-6 py-3 bg-gradient-to-r from-red-500 to-pink-500 text-white rounded-xl hover:from-red-600 hover:to-pink-600 transition-all duration-200 shadow-lg"
                    >
