...
export default function HelicopterSeatBooking() {
    ...
    return (
        <div className="min-h-screen bg-[#f8fafc] flex items-center justify-center p-4 md:p-8 font-sans">
            <div className="w-full max-w-5xl bg-white rounded-[2rem] border border-gray-100 p-8 md:p-12 relative overflow-hidden">

                {/* Back Button */}
                <button
                    onClick={() => router.back()}
                    className="absolute left-6 top-10 p-2.5 hover:bg-gray-100 rounded-full transition-all group z-30"
                >
                ...
                <div className="mb-10 pl-12">
                    <h1 className="text-3xl font-bold text-gray-900 tracking-tight mb-1.5">Select Seat</h1>
                    <p className="text-gray-500 text-base font-medium">Choose your preferred seat from the available options.</p>
                </div>

                {/* Main Interactive Map Container */}
                <div className="bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden mb-10">
                    {/* Visual Legend */}
                    <div className="flex flex-wrap justify-center items-center gap-8 py-6 bg-white border-b border-gray-50">
                    ...
                    <div className="relative flex justify-end py-20 bg-[#fafdff] px-4 overflow-x-auto scrollbar-hide">
                        <div
                            className="w-full min-w-[600px] max-w-4xl h-[320px] bg-no-repeat bg-contain bg-center relative flex items-center justify-center"
                            style={{ backgroundImage: "url('/flights/Helicop.svg')" }}
                        >
                            {/* Seat Placement Layout - Balanced in the cabin area */}
                            <div className="flex items-center gap-20 ml-120 mt-2">
                                    ...
                                    {pilotSeats.map((seatId) => (
                                        <div key={seatId} className="relative group">
                                            ...
                                            <img
                                                ...
                                                alt="Pilot Seat"
                                                className="w-6 h-6 object-contain opacity-90 transition-transform group-hover:scale-110"
                                            />
                                        </div>
                                    ))}
                                </div>
                ...
                <div className="flex flex-col lg:flex-row items-center justify-between gap-8 pt-6">
                    ...
                    <div className="flex flex-wrap gap-4 w-full lg:w-auto">
                        {passengers.map((p, idx) => (
                            <button
                                ...
                                className={cn(
                                    "flex items-stretch overflow-hidden rounded-xl border-2 transition-all duration-300 group min-w-[200px]",
                            ...
                            >
                                <div className="flex-1 px-5 py-4 text-left">
                                    <h3 className={cn("text-base font-bold mb-0.5", activePassengerIndex === idx ? "text-white" : "text-blue-600")}>
                                        {p.name}
                                    </h3>
                                    <p className={cn("text-xs font-medium", activePassengerIndex === idx ? "text-white/80" : "text-gray-400 font-semibold")}>
                                        {p.seat ? "Seat Confirmed" : "Choose Seat"}
                                    </p>
                                </div>
                                <div className={cn(
                                    "px-3 flex items-center justify-center font-black text-sm min-w-[40px] transition-colors",
                    ...
                    <button
                        ...
                        className="w-full lg:w-auto bg-[#ff9f43] hover:bg-[#f39c12] text-white font-black py-4.5 px-20 rounded-2xl md:rounded-full transition-all duration-300 text-xl shadow-xl shadow-orange-100 hover:shadow-orange-200 active:scale-[0.98]"
                    >
                        Continue
                    </button>
                </div>
...
function Legend({ color, label }) {
    return (
        <div className="flex items-center gap-2.5">
            <div className={cn("w-3.5 h-3.5 rounded-sm shadow-sm", color)} />
